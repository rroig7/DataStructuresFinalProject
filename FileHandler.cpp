...
/**
 * Stores each row of the csv into a vector. Insert each vector row into 
 * @throws NullException if file could not open 
 * @return A vector containing each row in the csv. 
 */
...
const std::vector<std::vector<std::string>> &FileHandler::getFileVector() const {
    return fileVector;
}
